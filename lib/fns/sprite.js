...
FN.prototype = {
	storeAndGetImagePos : function storeAndGetImagePos( renderer, css, sprite ){
		...
		switch( repeat ){
			...
			case 'repeat-x' : // horizontal sprite
				...
				pos = { x: 0, y: image.size, size: info.width };
			...
			case 'repeat-y' : // vertical sprite
				...
				pos = { x: image.size, y: 0, size: info.height };
				image.size += info.width;
				break;

			case 'repeat'   : // no sprite!  Danger, Will Robinson!
				image = self.images[ 'new-' + (self.ix++) ] = {};
		...
		image.src  || ( image.src  = path.relative( cssDir, imgDir ) + 'sprite-' + (self.imgIx++) + '.' + info.format.toLowerCase() );
		...
		for( var n in self.images ){
			...
			if( img.box ){      // sprites
				...
				im.convert([
					...
					, '-draw'
					, "'" + img.images.map(function( val ){
						// TODO: repeat image N times for scaling
						return 'image over ' + val.pos.x + ',' + val.pos.y + ' 0,0 "' + val.src + '"';
					}).join(' ') + "'"
					, img.path
				]);
