...
path.relative = require( './../relativePath' );
...
function storeAndGetImagePos( renderer, css, sprite, cb ){
	...
	step(function(){
		...
		var cssDir = path.dirname( renderer.get( 'filename' ) + 'a' )
		  , imgDir = path.dirname( ( renderer.get( 'imgPath' ) || renderer.get( 'filename' ) ) + 'a' );
...
module.exports = {
	...
	fn : function( renderer, css, sprite, cb ){
		step(function(){
			...
			cb( null, [
				'url("' + img.src + '")',
				'background-position: ' + ( img.pos.x ? -img.pos.x + 'px' : 0 ) + ' ' + ( img.pos.y ? -img.pos.y + 'px' : 0 ) + ';'
			]);
		});
	},
	...
	finalize : function( cb ){
		step(function(){
			...
			for( var n in images ){
				...
				if( img.box ){      // sprites
// convert -size 100x60 xc:transparent -draw "image over 0,0 0,0 '/Users/markkah/Sites/node/stylus-images/tests/sprites/fish_left_8.gif'"  out.png
					var cmd
					im.convert(cmd = [
						  '-size'
